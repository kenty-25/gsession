...
# Debian にプリインストールされた `wheel` を手動削除（エラー回避）
RUN apt remove -y python3-wheel || true

# Python & pip のバージョン確認（デバッグ用）
RUN python3 --version && python3 -m pip --version

# PEP 668 に対応して pip をアップグレード
RUN python3 -m pip install --no-cache-dir --upgrade --force-reinstall pip setuptools wheel --break-system-packages
